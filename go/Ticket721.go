...
//
// Solidity: function ticketInfoStorage(uint256 ) view returns(uint8 state, uint256 ticket_type, string event_JID, address sale_address)
func (_Ticket721 *Ticket721Caller) TicketInfoStorage(opts *bind.CallOpts, arg0 *big.Int) (struct {
	State       uint8
	TicketType  *big.Int
	EventJID    string
	SaleAddress common.Address
}, error) {
	ret := new(struct {
		State       uint8
		TicketType  *big.Int
